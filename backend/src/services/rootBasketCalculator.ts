...
export function calculateOverview(
  ...
  previous: RootBasketChainSnapshot | null,
  ...
  const top10 = new Set(sortedStake.slice(0, 10).map((validator) => validator.hotkey));
  const currentRootValidators = new Set(snapshot.root_hotkeys);
  const holdings = holdingsBySubnet(snapshot);
  const previousHoldings = previous ? holdingsBySubnet(previous) : new Map<number, number>();
  const previousTime = previous?.timestamp_ms ?? 0;
  ...
  const rows: RootBasketSubnetRow[] = snapshot.subnets.map((subnet) => {
    ...
    const previousAlpha = previousHoldings.get(subnet.netuid) || 0;
    ...
    const latestIncome = [...history.values()].map((items) => items.at(-1)?.estimated_income_24h_tao ?? null)
      .filter((value): value is number => value !== null)
      .reduce((sum, value) => sum + value, 0);
    const targetBuy = latestIncome === 0 ? null : latestIncome * weightedTargetShare;
    const sourceIncome = points.at(-1)?.estimated_income_24h_tao ?? 0;
    const sourceSell = sourceIncome * Math.max(0, 1 - weightedTargetShare);
    ...
    return {
      ...
      top_validator_count: pointing.filter((validator) => top10.has(validator.hotkey)).length,
      weighted_target_share: weightedTargetShare,
      basket_alpha: currentAlpha,
      holding_change_1h_alpha: oneHourPoint ? currentAlpha - oneHourPoint.basket_alpha : previousTime ? currentAlpha - previousAlpha : null,
      holding_change_24h_alpha: dayPoint ? currentAlpha - dayPoint.basket_alpha : previousTime ? currentAlpha - previousAlpha : null,
